...
def parse(session, url):
    ...
    num_reviews = soup.find('span', class_='_3jEYFo-z').text# get text
    #num_reviews = num_reviews[1:-1] 
    num_reviews = num_reviews.replace(',', '')
    num_reviews = re.sub(r'[a-z]+', '', num_reviews) 
...
def get_reviews_ids(soup):
    ...
    if items:
        reviews_ids = [x.attrs['data-reviewid'] for x in items]#[::2] uncomment if you doesn't want all the database (time consuming)
